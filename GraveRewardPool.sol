...
contract GraveRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. GRAVE to distribute in the pool.
        uint256 lastRewardTime; // Last time that GRAVE distribution occurred.
    ...
    uint256[] public epochTotalRewards = [80 ether, 60 ether];
    ...
    function pendingGrave(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accGravePerSkull = accGravePerSkull.add(_graveReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _graveReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accGravePerSkull = pool.accGravePerSkull.add(_graveReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
