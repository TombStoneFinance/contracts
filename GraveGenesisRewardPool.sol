...
contract GraveGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accgravePerSkull; // Accumulated grave per Skull, times 1e18. See below.
        bool isStarted; // if lastRewardBlock has passed
    }

    struct PoolTaxInfo {
    uint256 taxRate; // Tax rate in basis points (1% = 100)
    bool hasTax; // Flag to indicate if the pool has tax
}
    ...
    mapping(uint256 => PoolTaxInfo) public poolTaxInfo;
  ...
  //  uint256 public constant TOTAL_REWARDS = 20 ether;
    ...
    constructor(
        ...
        if (grave != address(0)) Grave = IERC20(grave);
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accgravePerSkull: 0, isStarted: _isStarted}));
        if (_isStarted) {
            totalAllocPoint = totalAllocPoint.add(_allocPoint);
        }
        poolTaxInfo[poolInfo.length - 1] = PoolTaxInfo({
        taxRate: _taxRate,
        hasTax: _hasTax
    });
    }
    ...
    function pendingGrave(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accgravePerSkull = pool.accgravePerSkull;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 graveReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accgravePerSkull = accgravePerSkull.add(graveReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accgravePerSkull).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 graveReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accgravePerSkull = pool.accgravePerSkull.add(graveReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
...
function deposit(uint256 _pid, uint256 _amount) public {
    ...
    if (user.amount > 0) {
        uint256 _pending = user.amount.mul(pool.accgravePerSkull).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
    ...
    if (_amount > 0) {
        ...
        PoolTaxInfo storage taxInfo = poolTaxInfo[_pid];
        if (taxInfo.hasTax) {
            uint256 taxAmount = _amount.mul(taxInfo.taxRate).div(10000); 
            _amount = _amount.sub(taxAmount);
            pool.token.safeTransferFrom(_sender, taxWallet, taxAmount);
        }
    ...
    user.rewardDebt = user.amount.mul(pool.accgravePerSkull).div(1e18);
    ...
    // Safe grave transfer function, just in case if rounding error causes pool to not have enough grave.
    function safegraveTransfer(address _to, uint256 _amount) internal {
        uint256 graveBalance = Grave.balanceOf(address(this));
        if (graveBalance > 0) {
            if (_amount > graveBalance) {
                Grave.safeTransfer(_to, graveBalance);
            } else {
    ...
    function getTaxInfo(uint256 _pid) public view returns (bool hasTax, uint256 taxRate) {
    PoolTaxInfo storage taxInfo = poolTaxInfo[_pid];
    return (taxInfo.hasTax, taxInfo.taxRate);
   }
