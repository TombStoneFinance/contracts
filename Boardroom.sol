...
contract SkullWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        uint256 masonSkull = _balances[msg.sender];
        require(masonSkull >= amount, "Masonry: withdraw request greater than staked amount");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = masonSkull.sub(amount);
        skull.safeTransfer(msg.sender, amount);
    }
}

contract Masonry is SkullWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        MasonrySnapshot memory genesisSnapshot = MasonrySnapshot({time : block.number, rewardReceived : 0, rewardPerSkull : 0});
        masonryHistory.push(genesisSnapshot);

        withdrawLockupEpochs = 2; // Lock for 6 epochs (36h) before release withdraw
        rewardLockupEpochs = 1; // Lock for 3 epochs (18h) before release claimReward Grave ON STEROIDS
    ...
    function rewardPerSkull() public view returns (uint256) {
        return getLatestSnapshot().rewardPerSkull;
    }

    function earned(address mason) public view returns (uint256) {
        uint256 latestRPS = getLatestSnapshot().rewardPerSkull;
        uint256 storedRPS = getLastSnapshotOf(mason).rewardPerSkull;
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        uint256 prevRPS = getLatestSnapshot().rewardPerSkull;
        uint256 nextRPS = prevRPS.add(amount.mul(1e18).div(totalSupply()));

        MasonrySnapshot memory newSnapshot = MasonrySnapshot({
            time: block.number,
            rewardReceived: amount,
            rewardPerSkull: nextRPS
        });
